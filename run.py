...
def get_valid_task_number(manager, action):
    while True:
        ...
        if task_number.isdigit() and 1 <= int(task_number) <= len(manager.tasks):
            ...
            print(f"Invalid task number. Please enter a valid task number between 1 and {len(manager.tasks)}.")
...
def main():
    ...
    while True:
        ...
        if choice == 1:
            ...
            if manager.tasks:
                task_number = get_valid_task_number(manager, "mark as completed")
                manager.mark_task_completed(task_number)
            else:
                print("No tasks available to mark as completed.")
        elif choice == 4:
            if manager.tasks:
                task_number = get_valid_task_number(manager, "remove")
